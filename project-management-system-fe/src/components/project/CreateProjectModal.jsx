...
const CreateProjectModal = ({ isOpen, onClose, onProjectCreated }) => {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'name' && !isKeyManuallyEdited) {
      const newKey = value.trim().toUpperCase().split(' ').map(word => word.charAt(0)).join('').slice(0, 10);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name || !formData.key || !formData.type || !formData.projectLeaderId) {
        ...
        <form onSubmit={handleSubmit}>
          <div className="form-grid">
            <div className="form-group">
              <label htmlFor="name" className="form-label">Project Name <span className="required-star">*</span></label>
              <input type="text" name="name" id="name" value={formData.name} onChange={handleInputChange} className="form-input" required />
            </div>
            <div className="form-group">
              <label htmlFor="key" className="form-label">Key <span className="required-star">*</span></label>
              <input type="text" name="key" id="key" value={formData.key} onChange={handleInputChange} className="form-input" required />
            </div>
            <div className="form-group form-group-span-2">
              <label htmlFor="type" className="form-label">Type <span className="required-star">*</span></label>
            ...
            <div className="form-group form-group-span-2">
              <label htmlFor="projectLeaderId" className="form-label">Project Manager <span className="required-star">*</span></label>
              <select name="projectLeaderId" id="projectLeaderId" value={formData.projectLeaderId} onChange={handleInputChange} className="form-select" required>
          ...
          <div className="modal-actions">
            <button type="button" onClick={handleClose} className="modal-btn modal-btn-cancel">Cancel</button>
