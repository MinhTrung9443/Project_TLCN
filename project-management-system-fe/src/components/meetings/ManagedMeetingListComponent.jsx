            ...
            {selectedMeeting.relatedTeamId ? (
              <div className="border-t border-neutral-200 pt-4">
                ...
                <div className="p-3 rounded-lg bg-primary-50 border border-primary-200">
                  <p className="text-sm font-medium text-primary-900">{selectedMeeting.relatedTeamId.name}</p>
                  {selectedMeeting.relatedTeamId.description && (
                    <p className="text-xs text-primary-700 mt-1">{selectedMeeting.relatedTeamId.description}</p>
                  )}
                </div>
              </div>
            ) : null}

            {selectedMeeting.relatedTaskId ? (
              <div className="border-t border-neutral-200 pt-4">
                <h4 className="text-sm font-semibold text-neutral-800 mb-2">Related Task</h4>
                <div className="p-3 rounded-lg bg-neutral-100 border border-neutral-200">
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="text-sm font-medium text-neutral-900">{selectedMeeting.relatedTaskId.key}</p>
                      <p className="text-xs text-neutral-600 mt-1">{selectedMeeting.relatedTaskId.name}</p>
                    </div>
                    {selectedMeeting.relatedTaskId.priority && (
                      <span className="px-2 py-1 text-xs rounded bg-neutral-200 text-neutral-700">{selectedMeeting.relatedTaskId.priority}</span>
                    )}
                  </div>
                </div>
              </div>
